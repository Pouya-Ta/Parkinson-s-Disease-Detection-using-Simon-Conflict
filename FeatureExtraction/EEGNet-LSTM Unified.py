...
class Config:
    ...
    # Output files
    FINAL_FEATURE_FILENAME = "lstm_selected_features.npy"
    FINAL_LABEL_FILENAME = "final_labels.npy"
    FINAL_GROUPS_FILENAME = "final_groups.npy"
...
class EEGNet(nn.Module):
    ...
    def _get_feature_map_size(self, x):
        ...
        x = self.elu(self.batchnorm2(self.depthwise_conv(self.batchnorm1(self.conv1(x)))))
        x = self.avgpool1(x)
        x = self.elu(self.batchnorm3(self.separable_conv(x)))
        x = self.avgpool2(x)
        x = self.flatten(x)
        return x.shape[1]
...
def run_eegnet_feature_extraction(config, device):
    # Stage 1: train EEGNet via CV and get out-of-sample features.
    print("\n========== STAGE 1: EEGNet Feature Extraction ==========")
    ...
    # Figure out feature size before the loop
    dummy_model = EEGNet(n_classes=n_classes, chans=n_chans, samples=n_samples, config=config)
    with torch.no_grad():
        dummy_input = torch.zeros(1, 1, n_chans, n_samples)
        feature_size = dummy_model.get_features(dummy_input).shape[1]
    print(f"--- EEGNet feature size: {feature_size} ---")

    out_of_sample_features = np.zeros((X.shape[0], feature_size))
    out_of_sample_labels = np.zeros(y.shape)
    out_of_sample_groups = np.zeros(groups.shape)

    for fold, (train_idx, test_idx) in enumerate(sgkf.split(X, y, groups=groups)):
        print(f"\n--- Starting EEGNet Fold {fold + 1}/{config.N_SPLITS} ---")
        ...
        test_loader = DataLoader(test_dataset, batch_size=config.EEG_BATCH_SIZE, shuffle=False)

        model = EEGNet(n_classes=n_classes, chans=n_chans, samples=n_samples, config=config).to(device)
        optimizer = optim.Adam(model.parameters(), lr=config.EEG_LEARNING_RATE)
        ...
        for epoch in range(config.EEG_EPOCHS):
            ...
            print(f"  Epoch {epoch+1:02d}/{config.EEG_EPOCHS} | Train Loss: {avg_train_loss:.4f} | Val Acc: {val_acc:.4f} | Val Balanced Acc: {val_balanced_acc:.4f}")
            ...
            if val_balanced_acc > best_val_metric:
                ...
                if patience_counter >= config.EEG_PATIENCE:
                    print(f"  Early stopping at epoch {epoch+1}. Best Balanced Acc: {best_val_metric:.4f}")
                    break

        print("  Extracting features from test set using best model...")
        ...
        out_of_sample_features[test_idx] = np.concatenate(fold_features, axis=0)
        out_of_sample_labels[test_idx] = y_test
        out_of_sample_groups[test_idx] = groups[test_idx]

    print("\n--- EEGNet Feature Extraction Complete ---")
    return out_of_sample_features, out_of_sample_labels, out_of_sample_groups

def run_lstm_feature_selection(X_features, y_labels, config, device):
    # Stage 2: train LSTM on the EEGNet features and get final features.
    print("\n========== STAGE 2: LSTM Feature Selection ==========")
    ...
    train_loader = DataLoader(full_dataset, batch_size=config.LSTM_BATCH_SIZE, shuffle=True)

    print("--- Training LSTM model ---")
    lstm_input_dim = X_features.shape[1]
    model = FeatureSelectionLSTM(input_size=lstm_input_dim, config=config).to(device)
    ...
    for epoch in range(config.LSTM_EPOCHS):
        ...
        for xb, yb in train_loader:
            ...
            total_train_loss += loss.item()
        avg_train_loss = total_train_loss / len(train_loader)
        print(f"  Epoch {epoch+1:02d}/{config.LSTM_EPOCHS} | Train Loss: {avg_train_loss:.4f}")

    print("--- LSTM Training Complete ---")

    print("--- Extracting final features from trained LSTM ---")
    model.eval()

    extraction_loader = DataLoader(full_dataset, batch_size=config.LSTM_BATCH_SIZE, shuffle=False)

    all_features_list = []
    with torch.no_grad():
        for xb, _ in extraction_loader:
            features = model.extract_features(xb.to(device))
...
def main():
    ...
    # Stage 1
    eegnet_features, labels, groups = run_eegnet_feature_extraction(config, device)

    # Stage 2
    final_features = run_lstm_feature_selection(eegnet_features, labels, config, device)

    # Save results
    print("\n========== STAGE 3: Saving Final Results ==========")
    np.save(config.FINAL_FEATURE_FILENAME, final_features)
    np.save(config.FINAL_LABEL_FILENAME, labels)
    np.save(config.FINAL_GROUPS_FILENAME, groups)

    print(f"Saved LSTM features: {final_features.shape} to '{config.FINAL_FEATURE_FILENAME}'")
    print(f"Saved labels: {labels.shape} to '{config.FINAL_LABEL_FILENAME}'")
    print(f"Saved groups: {groups.shape} to '{config.FINAL_GROUPS_FILENAME}'")
    print("\n--- Done ---")
